<?php

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use App\Models\Pemasukan;
use App\Models\KategoriKeuangan;
use App\Models\Donatur;
use App\Models\Rekening;
use Illuminate\Http\Request;
use Illuminate\Support\Str;

/**
 * PemasukanApiController
 * CRUD lengkap pemasukan + upload bukti untuk Flutter.
 */
class PemasukanApiController extends Controller
{
    // ─── GET /api/v1/pemasukan ───────────────────────────────────────────────
    // Query params: q, from, to, kategori_id, per_page, page
    public function index(Request $request)
    {
        $query = Pemasukan::with(['kategori', 'donatur', 'rekening']);

        if ($request->filled('q')) {
            $q = $request->q;
            $query->where(function ($sub) use ($q) {
                $sub->where('keterangan', 'like', "%$q%")
                    ->orWhere('nomor_transaksi', 'like', "%$q%");
            });
        }

        if ($request->filled('from') && $request->filled('to')) {
            $query->whereBetween('tanggal', [$request->from, $request->to]);
        }

        if ($request->filled('kategori_id')) {
            $query->where('kategori_id', $request->kategori_id);
        }

        $perPage = min((int) ($request->per_page ?? 15), 100);
        $items   = $query->latest('tanggal')->paginate($perPage);

        return response()->json([
            'status' => true,
            'data'   => $items->map(fn($i) => $this->format($i)),
            'meta'   => [
                'current_page' => $items->currentPage(),
                'last_page'    => $items->lastPage(),
                'per_page'     => $items->perPage(),
                'total'        => $items->total(),
            ],
        ]);
    }

    // ─── GET /api/v1/pemasukan/{id} ──────────────────────────────────────────
    public function show($id)
    {
        $item = Pemasukan::with(['kategori', 'donatur', 'rekening'])->findOrFail($id);

        return response()->json([
            'status' => true,
            'data'   => $this->format($item),
        ]);
    }

    // ─── POST /api/v1/pemasukan ──────────────────────────────────────────────
    public function store(Request $request)
    {
        $data = $request->validate([
            'tanggal'     => 'required|date',
            'nominal'     => 'required|numeric|min:0',
            'kategori_id' => 'required|exists:kategori_keuangan,id',
            'donatur_id'  => 'nullable|exists:donaturs,id',
            'rekening_id' => 'nullable|exists:rekenings,id',
            'metode'      => 'nullable|string|max:100',
            'sumber_dana' => 'nullable|string|max:150',
            'keterangan'  => 'nullable|string|max:1000',
        ]);

        $data['nomor_transaksi'] = 'PM' . now()->format('Ymd') . Str::upper(Str::random(6));
        $data['created_by']      = $request->user()->id;

        $item = Pemasukan::create($data);

        return response()->json([
            'status'  => true,
            'message' => 'Pemasukan berhasil disimpan',
            'data'    => $this->format($item->load(['kategori', 'donatur', 'rekening'])),
        ], 201);
    }

    // ─── PUT /api/v1/pemasukan/{id} ──────────────────────────────────────────
    public function update(Request $request, $id)
    {
        $item = Pemasukan::findOrFail($id);

        $data = $request->validate([
            'tanggal'     => 'sometimes|date',
            'nominal'     => 'sometimes|numeric|min:0',
            'kategori_id' => 'sometimes|exists:kategori_keuangan,id',
            'donatur_id'  => 'nullable|exists:donaturs,id',
            'rekening_id' => 'nullable|exists:rekenings,id',
            'metode'      => 'nullable|string|max:100',
            'sumber_dana' => 'nullable|string|max:150',
            'keterangan'  => 'nullable|string|max:1000',
        ]);

        $item->update($data);

        return response()->json([
            'status'  => true,
            'message' => 'Pemasukan berhasil diperbarui',
            'data'    => $this->format($item->fresh(['kategori', 'donatur', 'rekening'])),
        ]);
    }

    // ─── DELETE /api/v1/pemasukan/{id} ───────────────────────────────────────
    public function destroy($id)
    {
        $item = Pemasukan::findOrFail($id);
        $item->delete();

        return response()->json([
            'status'  => true,
            'message' => 'Pemasukan berhasil dihapus',
        ]);
    }

    // ─── POST /api/v1/pemasukan/{id}/bukti ───────────────────────────────────
    // Flutter kirim file sebagai multipart/form-data field "bukti"
    public function uploadBukti(Request $request, $id)
    {
        $item = Pemasukan::findOrFail($id);

        $request->validate([
            'bukti' => 'required|file|mimes:jpg,jpeg,png,pdf|max:10240',
        ]);

        // Hapus file lama jika ada
        if ($item->bukti) {
            \Illuminate\Support\Facades\Storage::disk('public')->delete($item->bukti);
        }

        $path = $request->file('bukti')->store('pemasukan/bukti', 'public');
        $item->update(['bukti' => $path]);

        return response()->json([
            'status'   => true,
            'message'  => 'Bukti berhasil diunggah',
            'data'     => ['bukti_url' => asset('storage/' . $path)],
        ]);
    }

    // ─── Helper ─────────────────────────────────────────────────────────────
    private function format(Pemasukan $item): array
    {
        return [
            'id'              => $item->id,
            'nomor_transaksi' => $item->nomor_transaksi,
            'tanggal'         => $item->tanggal,
            'nominal'         => (int) $item->nominal,
            'metode'          => $item->metode,
            'sumber_dana'     => $item->sumber_dana,
            'keterangan'      => $item->keterangan,
            'bukti_url'       => $item->bukti ? asset('storage/' . $item->bukti) : null,
            'kategori'        => $item->kategori ? [
                'id'   => $item->kategori->id,
                'nama' => $item->kategori->nama,
            ] : null,
            'donatur'         => $item->donatur ? [
                'id'   => $item->donatur->id,
                'nama' => $item->donatur->nama,
            ] : null,
            'rekening'        => $item->rekening ? [
                'id'   => $item->rekening->id,
                'nama' => $item->rekening->nama,
            ] : null,
            'created_at'      => $item->created_at?->toDateTimeString(),
            'updated_at'      => $item->updated_at?->toDateTimeString(),
        ];
    }
}
