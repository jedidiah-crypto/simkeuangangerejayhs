<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;gap:1rem;">
            ...
            <div>
                <p style="font-size:0.72rem;font-weight:600;color:#06b6d4;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.2rem;">Edit Data</p>
                <h1 class="font-display" style="font-size:1.4rem;font-weight:700;color:#e2e8f0;margin:0;">Edit Donatur</h1>
            </div>
        </div>
    </x-slot>
            ...
            <div style="background:linear-gradient(135deg,rgba(6,182,212,0.12),rgba(34,211,238,0.05));padding:1.5rem;border-bottom:1px solid var(--border);">
                <div style="display:flex;align-items:center;gap:0.75rem;">
                    <div style="width:40px;height:40px;border-radius:12px;background:rgba(6,182,212,0.2);display:flex;align-items:center;justify-content:center;">
                        <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="#22d3ee"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                    </div>
                    <div>
                        <p style="font-size:0.875rem;font-weight:600;color:#e2e8f0;">{{ $donatur->nama }}</p>
                        <p style="font-size:0.75rem;color:#64748b;">Perbarui informasi donatur</p>
                    </div>
                </div>
            </div>

            <form action="{{ route('donatur.update', $donatur) }}" method="POST" style="padding:1.75rem;">
                @csrf
                @method('PUT')
                <div style="margin-bottom:1.25rem;">
                    <label class="form-label">Nama Donatur *</label>
                    <input type="text" name="nama" class="form-input" required placeholder="Masukkan nama donatur" value="{{ old('nama', $donatur->nama) }}">
                    @error('nama') <p style="font-size:0.75rem;color:#fb7185;margin-top:0.35rem;">{{ $message }}</p> @enderror
                </div>

                <div style="margin-bottom:1.25rem;">
                    <label class="form-label">Email</label>
                    <input type="email" name="email" class="form-input" placeholder="nama@contoh.com" value="{{ old('email', $donatur->email) }}">
                    @error('email') <p style="font-size:0.75rem;color:#fb7185;margin-top:0.35rem;">{{ $message }}</p> @enderror
                </div>

                <div style="margin-bottom:1.25rem;">
                    <label class="form-label">Telepon</label>
                    <input type="text" name="telepon" class="form-input" placeholder="08xxxxxxxxxx" value="{{ old('telepon', $donatur->telepon) }}">
                    @error('telepon') <p style="font-size:0.75rem;color:#fb7185;margin-top:0.35rem;">{{ $message }}</p> @enderror
                </div>

                <div style="margin-bottom:1.75rem;">
                    <label class="form-label">Alamat</label>
                    <textarea name="alamat" rows="3" class="form-textarea" placeholder="Alamat lengkap donatur...">{{ old('alamat', $donatur->alamat) }}</textarea>
