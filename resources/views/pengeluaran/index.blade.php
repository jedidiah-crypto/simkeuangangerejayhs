<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;">
            <div>
                <p style="font-size:0.72rem;font-weight:600;color:#f43f5e;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.2rem;">Manajemen Dana</p>
                <h1 class="font-display" style="font-size:1.4rem;font-weight:700;color:#1a1200;margin:0;">Daftar Pengeluaran</h1>
                <p style="font-size:0.8rem;color:#78716c;margin-top:0.2rem;">Catat dan kelola status pengeluaran gereja</p>
            </div>
            <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
                ...
                <a href="{{ route('pengeluaran.create') }}" class="btn-danger" style="font-size:0.8rem;">
    ...
    <div class="glass-card animate-in" style="padding:1rem 1.5rem;margin-bottom:1rem;">
        <form method="GET" action="{{ route('pengeluaran.index') }}" style="display:flex;gap:0.75rem;flex-wrap:wrap;align-items:flex-end;">
            <div>
            ...
            @if(request()->hasAny(['q','from','to']))
                <a href="{{ route('pengeluaran.index') }}" class="btn-ghost" style="font-size:0.82rem;">Reset</a>
            @endif
        </form>
    </div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            <td>
                                <span style="font-family:'Sora',sans-serif;font-size:0.8rem;font-weight:600;color:#f43f5e;background:rgba(244,63,94,0.08);padding:0.25rem 0.6rem;border-radius:8px;border:1px solid rgba(244,63,94,0.15);">
                            ...
                            <td style="color:#57534e;font-size:0.83rem;">{{ $item->metode ?? '-' }}</td>
                            <td>
                            ...
                            <td>
                                @if($item->nota)
                                    <a href="{{ asset('storage/'.$item->nota) }}" target="_blank" style="display:inline-flex;align-items:center;gap:0.3rem;font-size:0.78rem;color:#38bdf8;text-decoration:none;background:rgba(56,189,248,0.08);padding:0.25rem 0.6rem;border-radius:8px;border:1px solid rgba(56,189,248,0.15);transition:all 0.15s;" onmouseover="this.style.background='rgba(56,189,248,0.15)'" onmouseout="this.style.background='rgba(56,189,248,0.08)'">
                            ...
                            <td>
                                <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
                                    <a href="{{ route('pengeluaran.edit', $item) }}" style="display:inline-flex;align-items:center;gap:0.3rem;font-size:0.75rem;color:#fbbf24;text-decoration:none;background:rgba(251,191,36,0.08);padding:0.3rem 0.6rem;border-radius:6px;border:1px solid rgba(251,191,36,0.2);transition:all 0.15s;" onmouseover="this.style.background='rgba(251,191,36,0.15)'" onmouseout="this.style.background='rgba(251,191,36,0.08)'">
                                    ...
                                    <form action="{{ route('pengeluaran.destroy', $item) }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin ingin menghapus pengeluaran ini?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" style="text-align:center;padding:3rem;color:#78716c;">
