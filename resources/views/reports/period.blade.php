<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;">
            <div>
                ...
                <h1 class="font-display" style="font-size:1.4rem;font-weight:700;color:#1a1200;margin:0;">Laporan Periode</h1>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1rem;margin-bottom:1.5rem;" class="animate-in">
        <div class="stat-card stat-emerald">
            ...
            <p style="font-size:1.5rem;font-weight:800;color:#1a1200;margin-top:0.4rem;font-family:'Sora',sans-serif;">Rp {{ number_format($totalP,0,',','.') }}</p>
            <p style="font-size:0.75rem;color:#78716c;margin-top:0.5rem;">{{ $pemasukan->count() }} transaksi</p>
        </div>
        <div class="stat-card stat-rose">
            ...
            <p style="font-size:1.5rem;font-weight:800;color:#1a1200;margin-top:0.4rem;font-family:'Sora',sans-serif;">Rp {{ number_format($totalPg,0,',','.') }}</p>
            <p style="font-size:0.75rem;color:#78716c;margin-top:0.5rem;">{{ $pengeluaran->count() }} transaksi</p>
        </div>
        <div class="stat-card stat-violet">
            <p style="font-size:0.72rem;font-weight:600;color:#7c3aed;text-transform:uppercase;letter-spacing:0.08em;">Selisih Periode</p>
            ...
            <p style="font-size:0.75rem;color:#78716c;margin-top:0.5rem;">{{ $selisih >= 0 ? 'Surplus' : 'Defisit' }}</p>
        </div>
    </div>
    ...
    <div class="glass-card animate-in" style="margin-bottom:1.5rem;overflow:hidden;">
        <div style="padding:1rem 1.5rem;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:0.5rem;">
            ...
            <h3 class="font-display" style="font-size:0.88rem;font-weight:700;color:#1a7a4a;margin:0;">Pemasukan</h3>
            <span style="margin-left:auto;font-size:0.75rem;color:#78716c;">{{ $pemasukan->count() }} transaksi</span>
        </div>
        <div style="overflow-x:auto;">
            <table class="data-table">
                ...
                <tbody>
                    @forelse($pemasukan as $item)
                        <tr>
                            ...
                            <td style="color:#57534e;font-size:0.82rem;">{{ \Carbon\Carbon::parse($item->tanggal)->isoFormat('D MMM Y') }}</td>
                            <td style="color:#57534e;font-size:0.82rem;">{{ $item->kategori->nama ?? '-' }}</td>
                            <td style="color:#57534e;font-size:0.82rem;">{{ $item->sumber_dana ?? '-' }}</td>
                            <td style="color:#57534e;font-size:0.82rem;max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $item->keterangan ?? '-' }}</td>
                            <td><span class="amount-in" style="font-family:'Sora',sans-serif;">+Rp {{ number_format($item->nominal,0,',','.') }}</span></td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" style="text-align:center;padding:2.5rem;color:#78716c;">
                ...
                @if($pemasukan->count() > 0)
                    <tfoot>
                        <tr>
                            <td colspan="5" style="padding:0.75rem 1rem;text-align:right;font-size:0.82rem;font-weight:600;color:#57534e;border-top:1px solid rgba(180,140,50,0.1);">Total Pemasukan</td>
                            <td style="padding:0.75rem 1rem;border-top:1px solid rgba(180,140,50,0.1);">
    ...
    <div class="glass-card animate-in" style="overflow:hidden;">
        <div style="padding:1rem 1.5rem;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:0.5rem;">
            ...
            <h3 class="font-display" style="font-size:0.88rem;font-weight:700;color:#b91c1c;margin:0;">Pengeluaran</h3>
            <span style="margin-left:auto;font-size:0.75rem;color:#78716c;">{{ $pengeluaran->count() }} transaksi</span>
        </div>
        <div style="overflow-x:auto;">
            <table class="data-table">
                <thead>
                ...
                <tbody>
                    @forelse($pengeluaran as $item)
                        <tr>
                            ...
                            <td style="color:#57534e;font-size:0.82rem;">{{ \Carbon\Carbon::parse($item->tanggal)->isoFormat('D MMM Y') }}</td>
                            <td style="color:#57534e;font-size:0.82rem;">{{ $item->kategori->nama ?? '-' }}</td>
                            <td style="color:#57534e;font-size:0.82rem;">{{ $item->metode ?? '-' }}</td>
                            <td style="color:#57534e;font-size:0.82rem;max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $item->keterangan ?? '-' }}</td>
                ...
                @if($pengeluaran->count() > 0)
                    <tfoot>
                        <tr>
                            <td colspan="5" style="padding:0.75rem 1rem;text-align:right;font-size:0.82rem;font-weight:600;color:#57534e;border-top:1px solid rgba(180,140,50,0.1);">Total Pengeluaran</td>
                            <td style="padding:0.75rem 1rem;border-top:1px solid rgba(180,140,50,0.1);">
