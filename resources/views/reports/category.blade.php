<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;">
            <div>
                ...
                <h1 class="font-display" style="font-size:1.4rem;font-weight:700;color:#1a1200;margin:0;">Laporan per Kategori</h1>
    ...
    <div style="margin-bottom:2rem;">
        <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1rem;">
            ...
            <h2 style="color:#1a1200;font-size:1.1rem;font-weight:700;margin:0;">📈 Pemasukan</h2>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:1rem;">
            @forelse($pemasukan as $kategori => $total)
                <div class="glass-card" style="padding:1.25rem;border-top:3px solid #34d399;">
                    <p style="font-size:0.8rem;color:#57534e;margin:0 0 0.5rem;">{{ $kategori }}</p>
                    ...
                    <div style="border-top:1px solid rgba(180,140,50,0.1);padding-top:0.75rem;">
                        <p style="font-size:0.7rem;color:#78716c;margin:0;">{{ number_format(($total/$totalPemasukan)*100,1,',','.') }}% dari total</p>
                    </div>
                </div>
            @empty
                <div style="grid-column:1/-1;text-align:center;padding:2rem;">
                    <p style="color:#78716c;">Tidak ada data pemasukan</p>
                </div>
            @endforelse
        </div>
    </div>
    ...
    <div>
        <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1rem;">
            ...
            <h2 style="color:#1a1200;font-size:1.1rem;font-weight:700;margin:0;">📉 Pengeluaran</h2>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:1rem;">
            @forelse($pengeluaran as $kategori => $total)
                <div class="glass-card" style="padding:1.25rem;border-top:3px solid #fb7185;">
                    ...
                    <div style="border-top:1px solid rgba(180,140,50,0.1);padding-top:0.75rem;">
                        <p style="font-size:0.7rem;color:#78716c;margin:0;">{{ number_format(($total/$totalPengeluaran)*100,1,',','.') }}% dari total</p>
                    </div>
                </div>
            @empty
                <div style="grid-column:1/-1;text-align:center;padding:2rem;">
                    <p style="color:#78716c;">Tidak ada data pengeluaran</p>
                </div>
            @endforelse
        </div>
    </div>
    ...
    @if($pemasukan->count() > 0 || $pengeluaran->count() > 0)
        <div class="glass-card animate-in" style="margin-top:2rem;overflow:hidden;">
            <div style="padding:1rem 1.5rem;border-bottom:1px solid var(--border);">
                <h3 style="color:#1a1200;font-size:0.9rem;font-weight:600;margin:0;">Ringkasan Kategori</h3>
            </div>
